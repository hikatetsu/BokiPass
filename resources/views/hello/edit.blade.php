...
@section('content')
  ...
  <form action="{{route('edit', ['post_id' => $post->id])}}" method="post" class="container mb-4">
    ...
    <div class="container form-group rounded-lg shadow-sm p-3 bg-white">
      ...
      <input type="month" name="pass_date" id="pass_date" value="{{old('pass_date',$post->pass_date)}}" class="w-100" placeholder="記載例：2021年01月（年月）">
      <small class="form-text text-muted">機種によっては年月日の選択画面が出ますが、日は登録時に切り捨てられ年月までが登録されます</small>
    </div>
    <div class="container form-group rounded-lg shadow-sm p-3 bg-white">
