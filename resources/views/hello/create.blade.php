...
@section('content')
  <div class="col-md-8 container">
    ...
    <!-- 合格体験記入力フォーム -->
    <form action="{{route('create')}}" method="post" class=" mb-4" enctype="multipart/form-data">
      @csrf
      ...
      <div class=" form-group rounded-lg shadow-sm p-3 bg-white">
        ...
        <input type="month" name="pass_date" id="pass_date" value="{{old('pass_date',date("Y-m"))}}" class=" p-1 input-pass-date" placeholder="記載例：2021-01(半角)">
      </div>
      <div class=" form-group rounded-lg shadow-sm p-3 bg-white">
        <label for="study_period" class="w-100 font-weight-bold">勉強期間(時間)はどれくらいでしたか？</label><br>
        <textarea name="study_period" id="study_period" rows="5" placeholder="記載例：何ヶ月間や合計何時間など。平日は何時間で休日は何時間など。" class="w-100 p-1">{{old('study_period')}}</textarea>
        <small class="form-text text-muted">191文字まで</small>
      </div>
      <div class=" form-group rounded-lg shadow-sm p-3 bg-white">
        <label for="study_method" class="w-100 font-weight-bold">どのような勉強法でしたか？</label><br>
        <textarea name="study_method" id="study_method" rows="5" placeholder="記載例：独学or通信講座or通学？通信講座や通学ならスクール名など。その他具体的な勉強法。" class="w-100 p-1">{{old('study_method')}}</textarea>
        <small class="form-text text-muted">191文字まで</small>
      </div>
      <div class=" form-group rounded-lg shadow-sm p-3 bg-white">
        <label for="books_used" class="w-100 font-weight-bold">使用した教材は何ですか？</label><br>
        <textarea name="books_used" id="books_used" rows="5" placeholder="記載例：教材やWebサービスの名前。それらの特徴など。" class="w-100 p-1">{{old('books_used')}}</textarea>
        <small class="form-text text-muted">191文字まで</small>
      </div>
      <div class=" form-group rounded-lg shadow-sm p-3 bg-white">
        <label for="advice" class="w-100 font-weight-bold">合格の秘訣や受験生へアドバイスをお願いします。</label><br>
        <textarea name="advice" id="advice" rows="5" placeholder="記載例：おすすめの勉強法や受験上の注意点など。" class="w-100 p-1">{{old('advice')}}</textarea>
        <small class="form-text text-muted">191文字まで</small>
      </div>
      <div class=" form-group rounded-lg shadow-sm p-3 bg-white">
        <label for="image" class="w-100 font-weight-bold">画像を添付（任意）</label><br>
        <input type="file" name="image" accept="image/png, image/jpeg" id="image" class="w-100">
